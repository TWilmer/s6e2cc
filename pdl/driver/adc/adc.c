...
static void Adc_InitNvic(volatile stc_adcn_t* pstcAdc)
{
    ...
    if (pstcAdc == (volatile stc_adcn_t*)(&ADC2))
    {
        ...
        NVIC_ClearPendingIRQ(ADC2_LCD_IRQn);
        NVIC_EnableIRQ(ADC2_LCD_IRQn);
        NVIC_SetPriority(ADC2_LCD_IRQn, PDL_IRQ_LEVEL_ADC2_LCD);      
    #else
...
static void Adc_DeInitNvic(volatile stc_adcn_t* pstcAdc)
{
    ...
    #if (PDL_MCU_CORE == PDL_FM0P_CORE) || (PDL_MCU_CORE == PDL_FM3_CORE)
        NVIC_ClearPendingIRQ(ADC2_LCD_IRQn);
        NVIC_DisableIRQ(ADC2_LCD_IRQn);
        NVIC_SetPriority(ADC2_LCD_IRQn, PDL_DEFAULT_INTERRUPT_LEVEL);      
    #else
        NVIC_ClearPendingIRQ(ADC2_IRQn);
                      ...
                      stc_adc_config_t*    pstcConfig )
{
    ...
    pstcAdc->ADSS0 = (uint8_t)(0x000000FFul & pstcConfig->u32SamplingTimeSelect.u32AD_CHn);
    pstcAdc->ADSS1 = (uint8_t)((0x0000FF00ul & pstcConfig->u32SamplingTimeSelect.u32AD_CHn)  >> 8u);
    pstcAdc->ADSS2 = (uint8_t)((0x00FF0000ul &  pstcConfig->u32SamplingTimeSelect.u32AD_CHn) >> 16u);
    pstcAdc->ADSS3 = (uint8_t)((0xFF000000ul &  pstcConfig->u32SamplingTimeSelect.u32AD_CHn) >> 24u);
    ...
    if(NULL != pstcConfig->pstcScanInit)
    {
        ...
        pstcAdc->SCIS0 = (uint8_t)(0x000000FFul & pstcConfig->pstcScanInit->u32ScanCannelSelect.u32AD_CHn);
        pstcAdc->SCIS1 = (uint8_t)((0x0000FF00ul & pstcConfig->pstcScanInit->u32ScanCannelSelect.u32AD_CHn) >> 8u);
        pstcAdc->SCIS2 = (uint8_t)((0x00FF0000ul & pstcConfig->pstcScanInit->u32ScanCannelSelect.u32AD_CHn) >> 16u);
        pstcAdc->SCIS3 = (uint8_t)((0xFF000000ul & pstcConfig->pstcScanInit->u32ScanCannelSelect.u32AD_CHn) >> 24u);
    ...
    if (NULL != pstcConfig->pstcRangeComparisonInit)
    {
        ...
        if (pstcConfig->pstcRangeComparisonInit->u8RangeComapreChannel > 31u)
        {
            return ErrorInvalidParameter;
        }

        // Set the channel compared (according to single channel compare mode)
        pstcAdc->WCMPSR_f.WCCH |= pstcConfig->pstcRangeComparisonInit->u8RangeComapreChannel;
