 ...
 ** @link HbifGroup HBIF Module description @endlink
 **
 ** History:
...
en_result_t Hbif_Init(stc_hbif_config_t* pstcConfig)
{
    ...
    FM_HYPERBUS->IEN_f.RPCINITE = pstcConfig->bEnableInterrupt;
    
    if (TRUE == pstcConfig->bTouchNvic)
    {
        NVIC_ClearPendingIRQ(HBIF_IRQn);
        NVIC_EnableIRQ(HBIF_IRQn);
        NVIC_SetPriority(HBIF_IRQn, PDL_IRQ_LEVEL_HBIF);  
    }
#endif    
...
en_result_t Hbif_DeInit(boolean_t bTouchNvic)
{
    ...
    {
        NVIC_ClearPendingIRQ(HBIF_IRQn);
        NVIC_DisableIRQ(HBIF_IRQn);
        NVIC_SetPriority(HBIF_IRQn, PDL_DEFAULT_INTERRUPT_LEVEL);   
    }
#endif    
...
boolean_t Hbif_GetStatus(en_hbif_status_t enStatus)
{
    ...
    switch(enStatus)
    {
        case HbifRstoDuringWrite:
            bRet = (FM_HYPERBUS->CSR_f.WRSTORERR == 1u) ? TRUE : FALSE;
            break;
        case HbifTransactionErrDuringWrite:
            bRet = (FM_HYPERBUS->CSR_f.WTSTOERR == 1u) ? TRUE : FALSE;
            break;
        ...
        case HbifRdsStallDuringRead:
            bRet = (FM_HYPERBUS->CSR_f.RDSTALL == 1u) ? TRUE : FALSE;
            break;
...
boolean_t Hbif_GetInterruptFlag(void)
{
    ...
    bFlag = (FM_HYPERBUS->ISR_f.RPCINITS == 1u) ? TRUE : FALSE;
