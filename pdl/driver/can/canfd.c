 ...
 ** @link CanfdGroup CAN FD Module description @endlink
 **
 ** History:
...
static void CanfdInitInterrupt( volatile stc_canfdn_t* pstcCanfd )
{
    NVIC_ClearPendingIRQ(CAN1_CANFD_IRQn);
    NVIC_EnableIRQ(CAN1_CANFD_IRQn);
    NVIC_SetPriority(CAN1_CANFD_IRQn, PDL_IRQ_LEVEL_CAN1_CANFD);
}
...
{
    NVIC_ClearPendingIRQ(CAN1_CANFD_IRQn);
    NVIC_DisableIRQ(CAN1_CANFD_IRQn);
    NVIC_SetPriority(CAN1_CANFD_IRQn, PDL_DEFAULT_INTERRUPT_LEVEL);
}
...
en_result_t Canfd_Init( volatile stc_canfdn_t* pstcCanfd, const stc_canfd_config_t* pstcConfig )
{
	...
	uint16_t					u16FBRP;			// CAN FD bit rate prescaler
	...
	uint16_t					u16FTSEG1;			// CAN FD Time segment 1
	uint16_t					u16FTSEG2;			// CAN FD Time segment 2
	uint16_t					u16FSJW;			// CAN FD (Re)synchronization jump width
	uint16_t					u16Count;
	en_result_t					enResult;
	...
	if ( Ok == enResult )
	{
		...
		pstcCanfd->CCCR_f.CME  = 2;	// 10：enable full CAN FD operation (long data, flexible Rx/Tx data rate)
								   ...
								   stc_canfd_rx_buffer_t* pstcRxBuffer )
{
	...
	else
	{
		...
		if ( 0 == unRXFC.RXFC_f.F0S ||
			 ( PDL_OFF == unRXFS.RXFS_f.F0F && unRXFS.RXFS_f.F0GI == unRXFS.RXFS_f.F0PI ) )
		{
			enResult = Error;
		}
		// Otherwise, ...
		else
		{
			...
			pstcMsg = (stc_canfd_rx_buffer_t*)((uint8_t*)pstcCanfd + CANFD_MSGRAM_OFFSET +
											   ((unRXFC.RXFC_f.F0SA + (CANFD_RXBUF_INFOSIZE + u8Size) * unRXFS.RXFS_f.F0GI) << 2));
