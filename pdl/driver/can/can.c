            ...
            default:
                ...
                if (CAN_MSG_DIR_RX == pstcCan->IF2ARB_f.DIR)
                {
                    ...
                    if (NULL != pstcCanInternData->stcCanInterruptHandling[u16MessageBufferNumber - 1].pstcMsgBuffer)
                    {
                        ...
                        if ( CAN_MSG_ID_EXTENDED != pstcCan->IF2ARB2_f.XTD )
                        {
...
en_result_t Can_DeInit( volatile stc_cann_t* pstcCan )
{
    ...
    pstcCan->IF1DTA1_L = 0x00u;
    pstcCan->IF1DTA2_L = 0x00u;
    pstcCan->IF1DTB1_L = 0x00u;
    pstcCan->IF1DTB2_L = 0x00u;
    pstcCan->IF2DTA1_L = 0x00u;
    pstcCan->IF2DTA2_L = 0x00u;
    pstcCan->IF2DTB1_L = 0x00u;
    pstcCan->IF2DTB2_L = 0x00u;
                                        ...
                                        )
{
    ...
    switch ( u8MsgBuf >> 3u ) // Which register to use (each reg holds 16 flags)
    {
        case 0:
