 ...
 **
 ** History:
 **   - 2014-11-21  0.1  EZh  First version for FM universal PDL.
 **
 *****************************************************************************/
...
static void MftFrtInitIrq( volatile stc_mftn_frt_t* pstcMft)
{
    ...
    NVIC_SetPriority(FRTIM_IRQn, PDL_IRQ_LEVEL_MFT_FRT);
#else
#if (PDL_INTERRUPT_ENABLE_MFT0_FRT == PDL_ON)    
    if(&MFT0_FRT == pstcMft)
    {
...
static void MftFrtDeInitIrq( volatile stc_mftn_frt_t* pstcMft)
{
    ...
    NVIC_SetPriority(FRTIM_IRQn, PDL_DEFAULT_INTERRUPT_LEVEL);   
#else
#if (PDL_INTERRUPT_ENABLE_MFT0_FRT == PDL_ON)      
    if(&MFT0_FRT == pstcMft)
    {
