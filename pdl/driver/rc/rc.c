 ...
 ** @link RcGroup Remote Control Module description @endlink
 **
 ** History:
...
{    
    ...
    NVIC_ClearPendingIRQ(CEC_IRQn);
    NVIC_EnableIRQ(CEC_IRQn);
    NVIC_SetPriority(CEC_IRQn, PDL_IRQ_LEVEL_CEC);
  #else
    #if (PDL_INTERRUPT_ENABLE_RC0 == PDL_ON)
...
en_result_t Rc_Tx_Cec_Init(volatile stc_rcn_t *pstcRc, 
                           stc_rc_tx_cec_config_t *pstcRcTxConfig)
{
    // Pointer to internal data
    stc_rc_intern_data_t* pstcRcInternData ; 
