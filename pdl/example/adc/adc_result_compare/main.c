 ...
 ** Main Module
 **
 ** \brief This example shows how to use the ADC result comparison with
 ** scan mode.
 **
 ...
 ** the settings. (input voltage is equal or great than setting.)
 ** The result is print to terminal I/O at the end.
 **
 ** History:
...
void Main_adc_compare( void )
{
    ...
    PDL_ZERO_STRUCT(stcCmpCfg);
    PDL_ZERO_STRUCT(stcIrqEn);
    PDL_ZERO_STRUCT(stcIrqCallBack);

    // Initialize interrupt
    stcIrqCallBack.pfnComparisonIrqCb = Main_adc_compare_callback;
    stcIrqEn.bComparisonIrq = TRUE;
    ...
    stcCmpCfg.bCompareAllChannels = FALSE;
    stcCmpCfg.bCompIrqEqualGreater = TRUE;  ///< input above setting,than trig.
    stcCmpCfg.u16CompareValue = ADC_CMP_VAL;
    stcCmpCfg.u8CompareChannel = ADC_INPUT_CH0;
...
///// For FM3, the calculation of sampling time and compare time is shown as following:
    ...
    stcConfig.pstcComparisonInit = &stcCmpCfg;
