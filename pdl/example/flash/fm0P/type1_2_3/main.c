...
int32_t main(void)
{
    ...
    i = 0x5a5a;
    MFlash_WriteData16Bit_Fm0Type3CrAndSecureArea((uint16_t*)0x100004,\
                            (uint16_t*)&i, 1);///< write flash in CR data area
#endif
    ...
    delay();
    MFlash_WriteData32Bit((uint32_t*)TEST_SECTOR1, (uint32_t*)u8TestData, sizeof(u8TestData)/sizeof(uint32_t),FALSE);
    for(i=0;i<TEST_SIZE;i++)
    {
        u8TestData[i] = 0xff;
    }
    ...
    for(i=0;i<TEST_SIZE;i++)
    {
        if((TEST_SIZE-i)&0xff != u8TestData[i])
        {
#ifdef DEBUG_PRINT  
