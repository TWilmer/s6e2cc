...
#define TEST_DUALSECTOR 0x508000
/******************************************************************************/
...
int32_t main(void)
{
    uint32_t i;
    uint8_t * pt;
    uint8_t *paddr = (uint8_t *)TEST_DUALSECTOR;
...
#endif
    delay();
    DFlash_SectorErase((uint16_t*)TEST_DUALSECTOR);
...
#ifdef DEBUG_PRINT
    printf("==================================================\n");
    printf("dual flash write 8bit data \n");
    printf("==================================================\n");
#endif
    delay();
    DFlash_WriteData8Bit((uint8_t*)TEST_DUALSECTOR, (uint8_t*)u8TestData, sizeof(u8TestData)/sizeof(uint8_t));
    for(i=0;i<TEST_SIZE;i++)
    {
        u8TestData[i] = 0xff;
    }
    ...
    for(i=0;i<TEST_SIZE;i++)
    {
        ...
        {
#ifdef DEBUG_PRINT  
            printf("dual flash 8bit write data fail \n");
#endif 
	    while(1);
        }  
    }
#ifdef DEBUG_PRINT
    printf("dual flash 8bit write data successfully \n");
#endif 
    delay();
...
#endif
    delay();
    DFlash_WriteData16Bit((uint16_t*)TEST_DUALSECTOR, (uint16_t*)u8TestData, sizeof(u8TestData)/sizeof(uint16_t));
    for(i=0;i<TEST_SIZE;i++)
    {
        u8TestData[i] = 0xff;
    }
    paddr = (uint8_t *)TEST_DUALSECTOR;
    pt = (uint8_t *)u8TestData;
    for(i=0;i<TEST_SIZE;i++)
    {
        *pt++ = *paddr++;
  ...
  delay();
    ...
    delay();
    DFlash_WriteData32Bit((uint32_t*)TEST_DUALSECTOR, (uint32_t*)u8TestData,sizeof(u8TestData)/sizeof(uint32_t));
    for(i=0;i<TEST_SIZE;i++)
    {
        u8TestData[i] = 0xff;
    }
