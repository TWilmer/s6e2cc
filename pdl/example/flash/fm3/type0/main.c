...
int32_t main(void)
{
   ...
   //Uart_Io_Init();
...
#endif
    delay();
    MFlash_WriteData16Bit((uint16_t*)TEST_SECTOR1, (uint16_t*)u8TestData, sizeof(u8TestData));
    for(i=0;i<TEST_SIZE;i++)
    {
        u8TestData[i] = 0xff;
    }
    ...
    delay();
    MFlash_WriteData32Bit((uint32_t*)TEST_SECTOR1, (uint32_t*)u8TestData, sizeof(u8TestData),FALSE);
    for(i=0;i<TEST_SIZE;i++)
    {
        u8TestData[i] = 0xff;
    }
