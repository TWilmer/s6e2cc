...
/** \file main.c
 **
 ** The example shows how to play wave data via I2S-Lite with interrupt
 ** callback mode.
 **
 ** History:
...
/**
 ******************************************************************************
 ** \brief  I2S TX FIFO callback function
 ******************************************************************************/
void I2sTxIrqCallback(void)
{
    uint8_t u8i;
    uint16_t u16OutLeft, u16OutRight;
    uint32_t u32Fifo;
  
    u8i = I2SL_FIFO_COUNT;  // Fill the FIFO 
    while(u8i--)
    {
        ...
        if(*pu16SoundData == 0xffff)
        {
          ...
          I2sl_DisableIrq(&I2slCh, I2slTxFifoIrq); // Disable TX FIFO interrupt
        }
    }
}
...
{ 
    ...
    stcI2slConfig.u8ClkInDiv = I2SL_CLK_DIV;        // PCLK division
